...
class ComponentTagCompiler
{
     ...
     * @param  \Bladezero\View\Compilers\BladeCompiler|null  $blade
     * @return void
     */
    ...
    protected function componentString(string $component, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            $view = Str::startsWith($component, 'mail::')
                ? "\$__env->getContainer()->make(Bladezero\\View\\Factory::class)->make('{$component}')"
                : "'$class'";
...
<?php if (isset($attributes) && $attributes instanceof Bladezero\View\ComponentAttributeBag && $constructor = (new ReflectionClass('.$class.'::class))->getConstructor()): ?>
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
    ...
    {
        $viewFactory = \Bladezero\Factory::getInstance();
     ...
     *
     * @param  \Bladezero\Contracts\View\Factory  $viewFactory
     * @param  string  $component
     * @return string|null
     */
        ...
        foreach ($this->blade->getAnonymousComponentPaths() as $path) {
            try {
                ...
                if (! is_null($guess = match (true) {
                    Factory::exists($guess = $path['prefixHash'].$delimiter.$formattedComponent) => $guess,
                    Factory::exists($guess = $path['prefixHash'].$delimiter.$formattedComponent.'.index') => $guess,
                    default => null,
                })) {
                    return $guess;
                }
    ...
    protected function guessAnonymousComponentUsingNamespaces(Factory $viewFactory, string $component)
    {
        return collect($this->blade->getAnonymousComponentNamespaces())
            ...
            ->reduce(function ($carry, $directory, $prefix) use ($component, $viewFactory) {
                ...
                if (Factory::exists($view = $this->guessViewName($componentName, $directory))) {
                    return $view;
                }

                if (Factory::exists($view = $this->guessViewName($componentName, $directory).'.index')) {
                    return $view;
                }
            });
    }
        ...
        return collect($attributes)
                ->map(function (string $value, string $attribute) use ($escapeBound) {
                    return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                                ? "'{$attribute}' => \Bladezero\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                                : "'{$attribute}' => {$value}";
                })
                ->implode(',');
    }
