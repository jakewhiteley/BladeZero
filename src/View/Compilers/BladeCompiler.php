...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                    ? "<?php if (\Rapier\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                    : "<?php if (\Rapier\Support\Facades\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\Rapier\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\Rapier\Support\Facades\Blade::check('{$name}')): ?>";
        });
